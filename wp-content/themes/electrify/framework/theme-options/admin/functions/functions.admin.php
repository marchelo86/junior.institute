...
function generate_options_css($newdata) {
	...
	$data = $newdata;
	$css_dir = THEME_CUSTOM_STYLES . '/'; // Shorten code, save 1 call
	
	ob_start(); // Capture all output (output buffering)
	require($css_dir . 'custom_styles.php'); // Generate CSS
	chmod($css_dir, 0775);
	chmod($css_dir . 'custom_styles.php', 0664);
	$css = str_replace(array("\r\n", "\r", "\n"),'',trim(ob_get_clean()));
	chmod(get_template_directory() . '/library/css/' . 'custom_styles.css', 0664);
	@file_put_contents(get_template_directory() . '/library/css/custom_styles.css', trim($css), LOCK_EX); // Save it
